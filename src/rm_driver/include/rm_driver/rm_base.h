...
/// 只要控制器运行正常并且目标角度在可达范围内，机械臂立即返回成功指令，此时机械臂可能仍在运行；
/// 若有错误，立即返回失败指令。
RM_BASESHARED_EXPORT int Movej_CANFD_With_Radio(SOCKHANDLE ArmSocket, const float *joint, bool follow, float expand, byte trajectory_mode, byte radio);
...
/// \return 0-成功，失败返回:错误码, rm_define.h查询.
///
RM_BASESHARED_EXPORT int Movep_CANFD_With_Radio(SOCKHANDLE ArmSocket, Pose pose, bool follow, byte trajectory_mode, byte radio);
